...
export function OAuthButtons({
  ...
  const { configs, isLoading } = usePublicConfigs()
  ...
  // 检查是否有任何 OAuth 提供商启用
  const hasAnyOAuthEnabled =
    configs?.["oauth.github.enabled"] ||
    configs?.["oauth.google.enabled"] ||
    configs?.["oauth.linuxdo.enabled"]

  // 如果没有任何 OAuth 提供商启用，返回 null
  if (!hasAnyOAuthEnabled) {
    return null
  }
